...
print("1. Liquid section ΔP (water, DN50/PN40, 100 m, Δz=0)")
res = march(inlet(water(50.0)), [Pipe(id="p", name="P", dn="DN50", pn="PN40", length_m=100.0)])
r = row_for(res, "P")
props = props_at(water(50.0), 10e5, 25.0)
D = PIPE_DATABASE["DN50"]["PN40"]
A = math.pi * D**2 / 4
...
res = march(inlet(gas(100.0)), [Pipe(id="p", name="G", dn="DN25", pn="PN40", length_m=50.0)])
r = row_for(res, "G")
...
res = march(inlet(water(50.0)), [
    Pipe(id="p1", name="A", dn="DN50", pn="PN40", length_m=10.0),
    Misc(id="m", name="HX", dp_kpa=20.0),
    Pipe(id="p2", name="B", dn="DN50", pn="PN40", length_m=10.0)])
...
res = march(inlet(tp), [Pipe(id="p", name="TP", dn="DN50", pn="PN40", length_m=30.0)],
...
res = march(inlet(koh, T=60.0), [Pipe(id="p", name="K", dn="DN50", pn="PN40", length_m=20.0)])
check("KOH denser than water & runs", prk["rho_l"] > 1100 and row_for(res, "K").dp_fric_kpa > 0,
      f"ρ_l={prk['rho_l']:.1f}")

# 9. suggest_dn ----------------------------------------------------------------
print("9. Auto-DN suggestion")
out = suggest_dn(water(80.0), 10e5, 25.0, pn="PN40",
